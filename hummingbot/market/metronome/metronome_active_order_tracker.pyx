...
cdef class MetronomeActiveOrderTracker:
    ...
    cdef tuple c_convert_diff_message_to_np_arrays(self, object message):
        ...
        if message_type == "trade_success":
            ...
            if side == "buy":
                if price in self._active_bids and order_id in self._active_bids[price]:
                    self._active_bids[price][order_id]["availableAmountBase"] -= float(message.content["amount_base"])
                    if self._active_bids[price][order_id]["availableAmountBase"] == 0:
                ...
                if price in self._active_asks and order_id in self._active_asks[price]:
                    self._active_asks[price][order_id]["availableAmountBase"] -= float(message.content["amount_base"])
                    if self._active_asks[price][order_id]["availableAmountBase"] == 0:
        ...
        if message_type == "receive":
            ...
            message.content["availableAmountBase"] = float(message.content["availableAmountBase"])
            if side == "buy":
    ...
    cdef tuple c_convert_snapshot_message_to_np_arrays(self, object message):
        ...
        for bid_order in message.content["bids"]:
            ...
            amount_base = bid_order["amount_base"]
            amount_quote = bid_order["amount_quote"]
            order_dict = {
                "availableAmountBase": Decimal(amount_base),
                "availableAmountQuote": Decimal(amount_quote),
                "orderId": order_id
            }
        ...
        for ask_order in message.content["asks"]:
            ...
            amount_base = ask_order["amount_base"]
            amount_quote = ask_order["amount_quote"]
            order_dict = {
                "availableAmountBase": Decimal(amount_base),
                "availableAmountQuote": Decimal(amount_quote),
                "orderId": order_id
    ...
    cdef np.ndarray[np.float64_t, ndim=1] c_convert_trade_message_to_np_array(self, object message):
        ...
        return np.array([message.timestamp, trade_type_value, float(price), float(message.content["amount_base"])],
                        dtype="float64")
